...
func RunNodeLifecycleTests(tester CloudControllerManagerTester) {
	SIGDescribe(feature.CloudProvider, framework.WithDisruptive(), "Nodes", func() {
		f := framework.NewDefaultFramework("cloudprovider")
		f.NamespacePodSecurityLevel = admissionapi.LevelPrivileged
		var c clientset.Interface
		var nlt NodeLifecycleTester

		ginkgo.BeforeEach(func() {
			t, ok := tester.NodeLifecycle()
			if !ok {
				e2eskipper.Skipf("tester %q does not implement node lifecycle iterface", tester.Name())
			}
			c = f.ClientSet
			nlt = t
		})
		...
		ginkgo.It("should be deleted on API server if it doesn't exist in the cloud provider", func(ctx context.Context) {
			...
			err = nlt.DeleteNodes([]string{nodeToDelete})
			if err != nil {
